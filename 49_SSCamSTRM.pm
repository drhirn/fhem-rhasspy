########################################################################################################################
# $Id: 49_SSCamSTRM.pm 22508 2020-08-01 08:40:15Z DS_Starter $
#########################################################################################################################
#       49_SSCamSTRM.pm
#
...
sub setVersionInfo {
  ...
  if($modules{$type}{META}{x_prereqs_src} && !$hash->{HELPER}{MODMETAABSENT}) {
      ...
      if($modules{$type}{META}{x_version}) {                                                       # {x_version} ( nur gesetzt wenn $Id: 49_SSCamSTRM.pm 22508 2020-08-01 08:40:15Z DS_Starter $ im Kopf komplett! vorhanden )
          $modules{$type}{META}{x_version} =~ s/1\.1\.1/$v/gx;
      } else {
          $modules{$type}{META}{x_version} = $v; 
      }
      return $@ unless (FHEM::Meta::SetInternals($hash));                                          # FVERSION wird gesetzt ( nur gesetzt wenn $Id: 49_SSCamSTRM.pm 22508 2020-08-01 08:40:15Z DS_Starter $ im Kopf komplett! vorhanden )
